    ...
    const handleRemoveOrder = async () => {
        try {
            onRemove(id)
                ...
                .finally(() => {
                    TOAST_SUCCESS(`Вы успешно удалили заказ-наряд №${id}`)
                    onCloseConfirmModal()
                })
        } catch (err) {
            console.error(err)
            TOAST_ERROR('Ошибка удаления заказ-наряда')
        }
    }
    return (
        <RemoveButtonTable
